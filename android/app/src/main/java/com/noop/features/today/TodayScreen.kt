...
fun TodayScreen(
    ...
    val date = runCatching {
        LocalDate.parse(snapshot.day).format(DateTimeFormatter.ofPattern("EEEE, d MMMM", Locale.getDefault()))
    }.getOrElse {
    ...
    LazyColumn(
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(NoopSpacing.xs)) {
                ...
                Text(
                    "Your body, translated into a plan for today.",
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(NoopSpacing.sm)) {
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(NoopSpacing.sm)) {
                    ...
                    MetricCard(
                        label = "Strain",
                        value = snapshot.strain?.let { String.format(Locale.US, "%.1f", it) } ?: "—",
