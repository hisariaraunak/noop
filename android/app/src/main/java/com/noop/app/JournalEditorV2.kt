...
internal fun JournalEditorV2(viewModel: AppViewModel) {
    ...
    LaunchedEffect(day) {
        ...
        runCatching {
            ...
            caffeine = merged["Caffeine"]?.numericValue?.takeIf(Double::isFinite)?.let { String.format(java.util.Locale.US, "%.0f", it) } ?: ""
    ...
    LazyColumn(
        ...
        item { Column(verticalArrangement = Arrangement.spacedBy(NoopSpacing.xs)) { Text("TODAY'S JOURNAL", style = NoopType.labelCaps, color = MaterialTheme.colorScheme.onSurfaceVariant); Text("Add context, not judgment", style = NoopType.editorialHeadline); Text("Your entries stay on-device and become useful when patterns repeat over time.", style = MaterialTheme.typography.bodyLarge, color = MaterialTheme.colorScheme.onSurfaceVariant) } }
        when {
            ...
            error != null -> item { RebuildStateCard("Journal unavailable", error ?: "Unknown error") }
                ...
                items(defaultJournalQuestions.size) { index ->
                    ...
                    JournalToggle(question, answers[question] == true) { enabled ->
                        ...
                        scope.launch { runCatching { viewModel.repo.upsertJournal(listOf(JournalEntry("noop-journal", day, question, enabled))) }.onFailure { error = it.message ?: "Could not save journal entry." } }
                    }
                }
                item {
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(NoopSpacing.sm)) {
                            ...
                            OutlinedTextField(
                                ...
                                onValueChange = { next ->
                                    ...
                                    scope.launch { runCatching { if (value == null) viewModel.repo.deleteJournalEntry("noop-journal", day, "Caffeine") else viewModel.repo.upsertJournal(listOf(JournalEntry("noop-journal", day, "Caffeine", true, numericValue = value))) }.onFailure { error = it.message ?: "Could not save caffeine." } }
                                },
                                label = { Text("mg today") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true, modifier = Modifier.fillMaxWidth(),
                            )
                        }
                    }
                }
...
private fun JournalToggle(label: String, checked: Boolean, onChange: (Boolean) -> Unit) {
    NoopSurface(level = NoopSurfaceLevel.Standard, modifier = Modifier.fillMaxWidth()) {
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) { Text(label, style = MaterialTheme.typography.bodyLarge); Switch(checked = checked, onCheckedChange = onChange) }
    }
}
